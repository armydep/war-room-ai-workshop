  ...
  if (incidents.length === 0) {
    return (
      <div className="card p-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Live Feed</h3>
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto">
        {incidents.map((incident) => (
          ...
          >
            ...
            <div className="text-sm font-medium text-gray-800 truncate">{incident.title}</div>
            <div className="text-xs text-gray-500 mt-0.5">{incident.source.replace('_', ' ')}</div>
          </button>
        ))}
      </div>
    </div>
